...
class PlayState extends MusicBeatState
{
	...
	public var startedScene:Bool = true;
	...
	public static var killedBySword:Bool = false;
	public static var swordGotten:Bool = false;
	public static var swordGottenCutscenePassed:Bool = false;
	...
	var bgLimo:BGSprite;
	var infoScreen:FlxSprite;
	var P1StandOff:FlxSprite;
	var P2StandOff:FlxSprite;
	...
	var santa:BGSprite;
	var waddleDeeBopper:BGSprite;
	var kirboBopper:BGSprite;
	var waddleDoo:BGSprite;
	var axeKnight:BGSprite;
	var bladeKnight:BGSprite;
	var javelinKnight:BGSprite;
	var maceKnight:BGSprite;
	var swordKnight:BGSprite;
	var tridentKnight:BGSprite;
	var spearDee:BGSprite;
	var bandanaDee:BGSprite;
	var heyTimer:Float;
	var EX:BGSprite;
	var attackScreen:FlxSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'halberd':
				...
				var orange:BGSprite = new BGSprite('Halberd/sky', -1200, -900, 0.9, 0.9);
				orange.setGraphicSize(Std.int(orange.width * 0.8));
				orange.updateHitbox();
				add(orange);

				sky = new BGSprite('Halberd/clouds', -1200, -1500, 0.9, 0.9, ['clouds move instance 1'], true);
				sky.setGraphicSize(Std.int(sky.width * 0.8));
				sky.updateHitbox();
				sky.dance();
				add(sky);

				var background:BGSprite = new BGSprite('Halberd/wall', -800, -1200, 0.9, 0.9);
				background.setGraphicSize(Std.int(background.width * 0.8));
				background.updateHitbox();
				add(background);

				var floor:BGSprite = new BGSprite('Halberd/floor', -800, -450, 0.9, 0.9);
				floor.setGraphicSize(Std.int(floor.width * 0.8));
				floor.updateHitbox();
				add(floor);
			case 'dreamland':
				var sky:BGSprite = new BGSprite('Dreamland/sky', -900, -300, 0.9, 0.9);
				sky.updateHitbox();
				add(sky);

				var castle:BGSprite = new BGSprite('Dreamland/castle', sky.x - 100, sky.y, 0.9, 0.9);
				castle.updateHitbox();
				add(castle);

				var ground:BGSprite = new BGSprite('Dreamland/floor', -1200, -800, 0.9, 0.9);
		...
		if (Paths.formatToSongPath(SONG.song) == 'star-warrior')
		{
			axeKnight = new BGSprite('theKnights/axe knight idle', gfGroup.x - 185, gfGroup.y + 10, 0.95, 0.95, ['axe knight idle instance 1'], true);
			add(axeKnight);
			swordKnight = new BGSprite('theKnights/sword knight idle', gfGroup.x + 500, gfGroup.y - 20, 0.95, 0.95, ['sword knight idle instance 1'], true);
			add(swordKnight);
		}
		if (Paths.formatToSongPath(SONG.song) == 'clobberin')
		{
			bandanaDee = new BGSprite('Dreamland/bandanadee', gfGroup.x - 150, gfGroup.y + 200, 0.95, 0.95, ['Bandana dee idle'], true);
			add(bandanaDee);
			spearDee = new BGSprite('Dreamland/waddle dee', gfGroup.x + 444, gfGroup.y + 200, 0.95, 0.95, ['speardee idle'], true);
			add(spearDee);
		}
		...
		if (Paths.formatToSongPath(SONG.song) == 'star-warrior')
		{
			maceKnight = new BGSprite('theKnights/mace knight idle', gfGroup.x - 550, gfGroup.y + 150, 0.95, 0.95, ['Mace knight idle'], true);
			add(maceKnight);
			tridentKnight = new BGSprite('theKnights/trident knight idle', gfGroup.x - 250, gfGroup.y + 450, 0.95, 0.95, ['trident knight idle'], true);
			add(tridentKnight);
			bladeKnight = new BGSprite('theKnights/blade knight idle', gfGroup.x + 400, gfGroup.y + 325, 0.95, 0.95, ['blade knight idle instance 1'], true);
			add(bladeKnight);
			javelinKnight = new BGSprite('theKnights/javelin knight idle', gfGroup.x + 400, gfGroup.y - 300, 0.95, 0.95, ['javelin knight idle'], true);
			add(javelinKnight);
		}
		...
		if (curStage == 'halberd')
		{
			attackScreen = new FlxSprite().makeGraphic(Std.int(FlxG.width * 10), Std.int(FlxG.height * 10), FlxColor.BLACK);
			attackScreen.screenCenter();
			attackScreen.scrollFactor.set();
			attackScreen.alpha = 0;
			add(attackScreen);
		}
		...
		if (Paths.formatToSongPath(SONG.song) == 'clobberin')
		{
			kirboBopper = new BGSprite('Dreamland/Kirby Bob', 1150, 825 - 300, 0.95, 0.95, ['kirby bob'], true);
			kirboBopper.flipX = true;
			add(kirboBopper);
			kirboBopper.dance();

			waddleDoo = new BGSprite('Dreamland/waddle doo', 1000, 900 - 300, 0.95, 0.95, ['waddle bob'], true);
			add(waddleDoo);
			waddleDoo.dance();

			waddleDeeBopper = new BGSprite('Dreamland/waddle dee bob', -450, 825 - 300, 0.95, 0.95, ['waddle dee bob'], true);
			waddleDeeBopper.flipX = true;
			add(waddleDeeBopper);
			waddleDeeBopper.dance();

			waddleDeeBopper = new BGSprite('Dreamland/waddle dee bob', -200, 900 - 300, 0.95, 0.95, ['waddle dee bob'], true);
			waddleDeeBopper.flipX = true;
			add(waddleDeeBopper);
			waddleDeeBopper.dance();
		}
		...
		if (boyfriend.curCharacter == 'bfsword')
			FlxG.save.data.swordGotten;
		...
		infoScreen = new FlxSprite();
		infoScreen.frames = Paths.getSparrowAtlas('info screen', 'shared');
		infoScreen.setGraphicSize(FlxG.width, FlxG.height);
		infoScreen.animation.addByPrefix('idle', 'infoscreenfae', 24, false);
		infoScreen.screenCenter();
		infoScreen.animation.play('idle');
		P1StandOff = new FlxSprite();
		P1StandOff.frames = Paths.getSparrowAtlas('standOff/panel2', 'shared');
		P1StandOff.animation.addByPrefix('idle', 'panel 2', 18, false);
		P1StandOff.screenCenter();
		P1StandOff.y += 295;
		P1StandOff.setGraphicSize(FlxG.width + 150, FlxG.height - 900);
		add(P1StandOff);
		P1StandOff.alpha = 0;
		P2StandOff = new FlxSprite();
		P2StandOff.frames = Paths.getSparrowAtlas('standOff/panel1', 'shared');
		P2StandOff.animation.addByPrefix('idle', 'panel', 18, false);
		P2StandOff.screenCenter();
		P2StandOff.y -= 300;
		P2StandOff.setGraphicSize(FlxG.width + 150, FlxG.height - 900);
		add(P2StandOff);
		P2StandOff.alpha = 0;
		dodgeWarning = new FlxSprite();
		dodgeWarning.frames = Paths.getSparrowAtlas('standOff/now', 'shared');
		dodgeWarning.animation.addByPrefix('idle', 'now', 18, true);
		dodgeWarning.screenCenter();
		dodgeWarning.y += 200;
		dodgeWarning.setGraphicSize(225, 225);
		add(dodgeWarning);
		dodgeWarning.alpha = 0;
		SLASH = new FlxSprite();
		SLASH.frames = Paths.getSparrowAtlas('standOff/SLASH!', 'shared');
		SLASH.animation.addByPrefix('idle', 'SLASH!', 18, false);
		SLASH.screenCenter();
		SLASH.setGraphicSize(FlxG.width + 150, FlxG.height + 100);
		add(SLASH);
		SLASH.alpha = 0;
		...
		doof.cameras = [camHUD];
		P1StandOff.cameras = [camHUD];
		P2StandOff.cameras = [camHUD];
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					...
					schoolIntro(doof);
				case 'dad-battle' | 'star-warrior':
					dad.playAnim('standoff', true);
					dad.specialAnim = true;
					inCutscene = true;
					var blackScreen:FlxSprite = new FlxSprite().makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.BLACK);

					add(blackScreen);
					EX = new BGSprite('Halberd/EX', gf.x + 100, dad.y - 300, 0.95, 0.95, ['ex'], true);
					add(EX);
					EX.dance();
					blackScreen.scrollFactor.set();
					FlxG.camera.zoom = 1;
					boyfriend.alpha = 0;
					snapCamFollowToPos(gf.x + 100, dad.y);
	...
	function startSong():Void
	{
		startingSong = false;
		killedBySword = false;
		swordGottenCutscenePassed = false;
		if (!swordGottenCutscenePassed && !swordGotten)
			swordGotten = false;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
		{
			...
			playerVocals = new FlxSound().loadEmbedded(Paths.playerVoices(PlayState.SONG.song));
		}
		else
		{
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ENTER && !startedScene)
		{
			camHUD.visible = false;
			startedScene = true;
			swordGotten = true;
			infoScreen.scrollFactor.set();
			boyfriend.alpha = 1;
			remove(EX);
			snapCamFollowToPos(boyfriend.x, boyfriend.y);
			boyfriend.playAnim('swordGotten');

			add(infoScreen);
			FlxTween.tween(infoScreen, {alpha: 0}, 3.2, {
				ease: FlxEase.linear,
				onComplete: function(twn:FlxTween)
				{
					{
		...
		if (FlxG.keys.justPressed.SPACE && !dodging)
		{
			dodge();
		}
	...
	{ // You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (daNote.noteType != 'Galaxia Note')
		{
			combo = 0;
		}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (note.hitCausesMiss || note.noteType == 'Galaxia Note')
			{
				noteMiss(note);
			...
			if (!note.noAnimation)
			{
				...
				if (note.noteType == 'Sword Note')
				{
					if (dad.hasBlockAnimations)
					{
						dad.playAnim(animToPlay + '-block', true);
						dad.holdTimer = 0;
					}

					if (boyfriend.hasAttackAnimations)
					{
						boyfriend.playAnim(animToPlay + '-attack', true);
						boyfriend.holdTimer = 0;
					}
				}
				if (note.gfNote)
	...
	function StandOff():Void
	{
		P1StandOff.alpha = 1;
		P2StandOff.alpha = 1;
		P1StandOff.animation.play('idle');
		P2StandOff.animation.play('idle');
		dad.playAnim('standoff', true);
		dad.specialAnim = true;
		FlxTween.tween(attackScreen, {alpha: 1}, 1, {
			ease: FlxEase.quadInOut,
			onComplete: function(twn:FlxTween)
			{
				trace('hi');
		...
		new FlxTimer().start(1, function(tmr:FlxTimer)
		{
			P1StandOff.alpha = 0;
			P2StandOff.alpha = 0;
			new FlxTimer().start(FlxG.random.float(0.5, 3), function(tmr:FlxTimer)
			{
				new FlxTimer().start(0.01, function(tmr:FlxTimer)
				{
					dodgeWarning.alpha = 1;
					dodgeWarning.animation.play('idle');
				});
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					dad.playAnim("singLEFT-attack", true);
					dad.specialAnim = true;
					dodgeWarning.alpha = 0;
					SLASH.alpha = 1;
					SLASH.animation.play('idle');
					if (!dodging)
					{
						killedBySword = true;
						health -= 999999;
					...
					new FlxTimer().start(1, function(tmr:FlxTimer)
					{
						FlxTween.tween(attackScreen, {alpha: 0}, 0.5, {
							ease: FlxEase.quadInOut,
							onComplete: function(twn:FlxTween)
							{
								trace('are ya winnin son?');
							}
						});
						SLASH.alpha = 0;
					});
